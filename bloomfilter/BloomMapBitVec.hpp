...
#ifndef BLOOMMAPBITVECT_HPP_
#define BLOOMMAPBITVECT_HPP_
...
//	 */
//	BloomMapBitVec<T>(BloomMap<T> &bloomMap) :
...
//	}
	 ...
	 */
	BloomMapBitVec<T>(size_t expectedElemNum, double fpr, const unsigned hashNum,
			const unsigned kmerSize, sdsl::bit_vector &bv, size_t unique) :
	...
	BloomMapBitVec<T>(const string &filterFilePath) {
...
private:
	...
	void writeHeader(ofstream &out) const {
		...
		header.hlen = sizeof(struct FileHeader) + m_kmerSize * m_hashNum;
...
//		}
	 ...
	 */
	inline vector<vector<unsigned> > parseSeedString(
...
#pragma omp atomic
	...
	uint64_t m_nEntry;
	uint64_t m_tEntry;
	const unsigned m_hashNum;
	const unsigned m_kmerSize;
	const char* MAGIC = "BLOOMMBV";
};

#endif /* BLOOMMAPBITVECT_HPP_ */
